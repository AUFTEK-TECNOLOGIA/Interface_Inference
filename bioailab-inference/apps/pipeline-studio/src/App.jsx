...
const TRAINING_ALGORITHM_ITEMS = [
...
// Regressões Matemáticas - Alternativa simples a modelos ML
...
const TRAINING_ALGO_PARAM_SCHEMA = {
  ridge: {
    fields: [
...
function App() {
  ...
  const [selectedNodes, setSelectedNodes] = useState([]); // Múltiplos nós selecionados
  const [selectedEdge, setSelectedEdge] = useState(null); // Edge selecionada para remoção/edição
  const [clipboard, setClipboard] = useState({ nodes: [], edges: [] }); // Clipboard para copiar/colar
  const [configFieldErrors, setConfigFieldErrors] = useState({});
  const [pipelineName, setPipelineName] = useState("pipeline_visual");
  const [workspace, setWorkspace] = useState(null); // { tenant, pipeline }
  ...
  const [duplicateModal, setDuplicateModal] = useState({
    open: false,
    source: null, // { tenant, pipeline, title }
    tenant: "",
    logoFile: null,
  });
  const [editModal, setEditModal] = useState({
    open: false,
    target: null, // { tenant, pipeline, title }
  });
  ...
  const [deleteVersionModal, setDeleteVersionModal] = useState({
    open: false,
    version: "",
  });
  const [renameVersionModal, setRenameVersionModal] = useState({
    open: false,
    version: "",
    name: "",
    reason: "",
  ...
  const [trainModal, setTrainModal] = useState({
    ...
    gridSearchManual: false,  // Modo grid search com seleção manual (salva todos os candidatos)
    changeReason: "",
    running: false,
    error: "",
    result: null,
    // Regressões matemáticas
    useRegression: false,  // Se true, usa regressão ao invés de ML
    regressionType: "linear",  // Tipo de regressão selecionada
    regressionAutoSelect: false,  // Se true, testa todos os tipos e seleciona o melhor
    polynomialDegree: 3,  // Grau do polinômio (se polynomial)
  });
  const [trainModelsDraft, setTrainModelsDraft] = useState({}); // stepId -> { enabled, algorithm, paramsText }
  ...
  const [trainingStudioOpen, setTrainingStudioOpen] = useState(false); // Novo Training Studio
  ...
  const [graphList, setGraphList] = useState([]); // Lista de todos os gráficos disponíveis
  const [graphIndex, setGraphIndex] = useState(0); // Índice atual na lista
  ...
  const [contextMenu, setContextMenu] = useState(null); // { x, y, nodeId, edgeId, type: 'node'|'edge' }
  const [useDefaultExperiment, setUseDefaultExperiment] = useState(false);
  const [viewport, setViewport] = useState({ x: 0, y: 0, zoom: 1 });
  const reactFlowWrapper = useRef(null);
  const reactFlowInstance = useRef(null); // Instância do ReactFlow para acesso à viewport
  ...
  const parseExperimentIdsText = useCallback((value) => {
  ...
  const buildTrainingParamsForAlgorithm = useCallback((algorithmKey) => {
  ...
  const getFlowColor = useCallback(
    (flowLabel) => {
      const label = flowLabel || t("flows.none");
  ...
  const getBlockCardCategory = useCallback((blockName) => {
    const name = String(blockName || "").toLowerCase();

    // Blocos de resposta: tratamos como "ml" para destacar como finalização/saída.
    if (name.startsWith("response_")) return "ml";

    // Dados / aquisição / extração
    if (name === "experiment_fetch" || name.endsWith("_extraction") || name.includes("fetch")) return "data";

    // Preparação / processamento
    if (
      name.includes("time_slice") ||
      name.includes("outlier") ||
    ...
    // ML
    if (name.includes("ml") || name.includes("inference") || name.includes("model") || name.includes("predict")) {
      return "ml";
    }
  ...
  const preparedSteps = useMemo(
    () =>
      nodes.map((node) => {
        // Encontrar todas as edges que entram neste nó
        const incomingEdges = edges.filter((edge) => edge.target === node.id);
        
        // Construir input_mapping a partir das edges
        // Formato: { "experiment_data": "experiment_fetch_1.experiment_data" }
        const inputMapping = {};
        incomingEdges.forEach((edge) => {
          // Extrair nome do output a partir do sourceHandle (formato: "blockName-out-outputName")
          const sourceHandle = edge.sourceHandle || "";
          const targetHandle = edge.targetHandle || "";
          
          // Parse do handle: "experiment_fetch-out-experiment_data" -> "experiment_data"
          const outputName = sourceHandle.includes("-out-") 
            ? sourceHandle.split("-out-")[1] 
            : sourceHandle;
          
          // Parse do handle de destino: "turbidimetry_extraction-in-experiment_data" -> "experiment_data"
          const inputName = targetHandle.includes("-in-")
            ? targetHandle.split("-in-")[1]
            : outputName;  // Se não tiver handle específico, usar o mesmo nome do output
  ...
  const resolveWorkspaceLogoSrc = useCallback((logo, tenant, pipeline) => {
    ...
    if (safeTenant && safePipeline) {
      return `${API_URL}/pipelines/workspaces/assets/${encodeURIComponent(safeTenant)}/${encodeURIComponent(safePipeline)}/${value.replace(/^\/+/, "")}`;
    }
    return value;
  }, []);

  const workspaceInitials = (w) => {
    const base = (w?.title || w?.pipeline || w?.tenant || "W").trim();
    const parts = base.split(/\\s+/).filter(Boolean);
    const letters =
      parts.length >= 2 ? `${parts[0][0] || ""}${parts[1][0] || ""}` : `${base[0] || ""}${base[1] || ""}`;
    ...
    <div className="app-container">
          ...
          <div className="workspace-home-card" role="dialog" aria-modal="true">
            ...
            <div className="workspace-home-grid">
              <div className="workspace-panel">
                <div className="workspace-panel-title">{t("workspace.createTitle")}</div>
                <div className="workspace-field">
              ...
              <div className="workspace-panel">
                <div className="workspace-panel-title">{t("workspace.loadTitle")}</div>
      ...
      <header>
        ...
        <div className="header-actions">
          ...
          />
          <button onClick={() => setWorkspaceHomeOpen(true)} className="btn-load" title={t("workspace.openTitle")} type="button">
            {t("workspace.open")}
          </button>
          <button onClick={savePipeline} disabled={!nodes.length} className="btn-save" title={t("app.saveTitle")}>
            {t("actions.save")}
          </button>
          <button onClick={downloadPipelineFile} disabled={!nodes.length} className="btn-load" title={t("workspace.downloadTitle")} type="button">
          ...
          <button
            ...
            className="btn-load"
            title={t("actions.autoLayoutTitle")}
            type="button"
          >
      ...
      <main className="five-column-layout">
        ...
        <aside className="panel-blocks" style={{ width: leftPanel.width }}>
...
{`{
  ...
  "predict_nmp_coliformes": 1.66
      ...
      {blockResultsModal.open && (
        <div className="helper-modal" onClick={closeBlockResultsModal} role="dialog" aria-modal="true">
          <div className="helper-modal-inner" onClick={(e) => e.stopPropagation()}>
            <div className="helper-modal-header">
              <div>
                <h4>{t("blockResults.title")}</h4>
                <div className="helper-modal-subtitle">{getStepLabel(blockResultsModal.stepId)}</div>
              </div>
              <button className="helper-modal-close" type="button" onClick={closeBlockResultsModal}>
                {t("actions.close")}
              </button>
            </div>
            ...
            ) : simulation?.step_results?.[blockResultsModal.stepId] ? (
              <div style={{ marginTop: 12 }}>
                <StepResultCard
                  stepId={blockResultsModal.stepId}
                  stepLabel={getStepLabel(blockResultsModal.stepId)}
                  stepData={simulation.step_results[blockResultsModal.stepId]}
                  onGraphClick={openGraphModal}
      ...
      {graphModalOpen && (
        <div className="graph-modal" onClick={closeGraphModal}>
          {/* Navegação: Seta Esquerda */}
          {graphList.length > 1 && (
            <button
              className="graph-modal-nav graph-modal-prev"
          ...
          <div className="graph-modal-inner" onClick={(e) => e.stopPropagation()}>
            <button className="graph-modal-close" onClick={closeGraphModal}>×</button>
            <h4>{graphModalTitle}</h4>
            <img src={graphModalSrc} alt={graphModalTitle} />
            {graphList.length > 1 && (
              <div className="graph-modal-counter">
                {graphIndex + 1} / {graphList.length}
              </div>
            )}
          </div>
          ...
          {graphList.length > 1 && (
            <button
              className="graph-modal-nav graph-modal-next"
              onClick={(e) => { e.stopPropagation(); handleNavigate(1); }}
              title="Próximo (→)"
            >
              ›
